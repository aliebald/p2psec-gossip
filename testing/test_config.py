...
class Test_config(unittest.TestCase):
    def test_missing_degree(self):
        # Check if an ValueError is raised when degree is missing
        generate_test_config(degree=None)
        self.__check_raises_valid_exception(ValueError)

    def test_missing_p2p_address(self):
        # Check if an ValueError is raised when p2p_address is missing
        generate_test_config(p2p_address=None)
        self.__check_raises_valid_exception(ValueError)

    def test_missing_api_address(self):
        # Check if an ValueError is raised when api_address is missing
        generate_test_config(api_address=None)
        self.__check_raises_valid_exception(ValueError)

    def test_missing_max_connections(self):
        # Check if an ValueError is raised when max_connections is missing
        generate_test_config(max_connections=None)
        self.__check_raises_valid_exception(ValueError)

    def test_missing_min_connections(self):
        # Check if an ValueError is raised when min_connections is missing
        generate_test_config(min_connections=None)
        self.__check_raises_valid_exception(ValueError)

    def test_missing_bootstrapper(self):
        # Check if an ValueError is raised when bootstrapper is missing
        generate_test_config(bootstrapper=None)
        self.__check_raises_valid_exception(ValueError)

    def test_invalid_cache_size(self):
        # Check if an ValueError is raised when cache_size is negative
        generate_test_config(cache_size=-10)
        self.__check_raises_valid_exception(ValueError)

        # Check if an ValueError is raised when cache_size = 0
        generate_test_config(cache_size=0)
        self.__check_raises_valid_exception(ValueError)

    def test_invalid_degree(self):
        # Check if an ValueError is raised when degree is negative
        generate_test_config(degree=-10)
        self.__check_raises_valid_exception(ValueError)

        # Check if an ValueError is raised when degree = 0
        generate_test_config(degree=0)
        self.__check_raises_valid_exception(ValueError)

    def test_invalid_min_connections(self):
        # Check if an ValueError is raised when min_connections is negative
        generate_test_config(min_connections=-10)
        self.__check_raises_valid_exception(ValueError)

    def test_invalid_max_connections(self):
        # Check if an ValueError is raised when max_connections is negative
        generate_test_config(max_connections=-10)
        self.__check_raises_valid_exception(ValueError)

        # Check if an ValueError is raised when max_connections < min_connections
        generate_test_config(max_connections=10, min_connections=20)
        self.__check_raises_valid_exception(ValueError)

        # Check if an ValueError is raised when max_connections = 0
        generate_test_config(max_connections=0)
        self.__check_raises_valid_exception(ValueError)

    def test_invalid_bootstrapper(self):
        # Check if an ValueError is raised when the port is missing
        generate_test_config(bootstrapper="127.0.0.1")
        self.__check_raises_valid_exception(ValueError)

        # Check if an ValueError is raised when the ip is too large
        generate_test_config(bootstrapper="127.0.0.0.1:100")
        self.__check_raises_valid_exception(ValueError)

        # Check if an ValueError is raised when the bootstrapper is a single
        # space
        generate_test_config(bootstrapper=" ")
        self.__check_raises_valid_exception(ValueError)

        # Check if an ValueError is raised when the bootstrapper is just a number
    ...
    def test_invalid_p2p_address(self):
        ...
        # Check if an ValueError is raised when the p2p_address is a single space
        generate_test_config(p2p_address=" ")
        self.__check_raises_valid_exception(ValueError)

        # Check if an ValueError is raised when the p2p_address is just a number
    ...
    def test_invalid_api_address(self):
        ...
        # Check if an ValueError is raised when the api_address is a single space
        generate_test_config(api_address=" ")
        self.__check_raises_valid_exception(ValueError)

        # Check if an ValueError is raised when the api_address is just a number
    ...
    def test_invalid_known_peers(self):
        ...
        # Check if an ValueError is raised when the known_peers is just a number
        generate_test_config(known_peers="127.0.0.1:100, 1234")
        self.__check_raises_valid_exception(ValueError)

        # Check if an ValueError is raised when the known_peers contains
        # duplicates
